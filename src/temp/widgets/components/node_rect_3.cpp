...
NodeRect3::NodeRect3(const int cardId_, QGraphicsItem *parent)
...
void NodeRect3::set(const QMap<InputVar, QVariant> &values) {
    updateInputVars(values);
}

void NodeRect3::setTitle(const QString &title) {
    titleItem->setPlainText(title);
    adjustChildItems();
}

void NodeRect3::setText(const QString &text) {
...
void NodeRect3::contextMenuEvent(QGraphicsSceneContextMenuEvent *event) {
    event->accept();
}

bool NodeRect3::sceneEventFilter(QGraphicsItem *watched, QEvent *event) {
...
void NodeRect3::mousePressEvent(QGraphicsSceneMouseEvent */*event*/) {
    // do nothing
...
void NodeRect3::mouseReleaseEvent(QGraphicsSceneMouseEvent *event) {
...
void NodeRect3::setUpContextMenu() {
    {
...
void NodeRect3::setUpConnections() {
...
void NodeRect3::installEventFilterOnChildItems() {
...
void NodeRect3::updateInputVars(const QMap<InputVar, QVariant> &values) {
...
void NodeRect3::adjustChildItems() {
    const auto enclosingRect = vars.getValue<QRectF>(InputVar::Rect);
    const auto borderWidth = vars.getValue<double>(InputVar::BorderWidth);
    const auto color = vars.getValue<QColor>(InputVar::Color);
...
QGraphicsView *NodeRect3::getView() const {
    if (scene() == nullptr)
        return nullptr;
...
QString NodeRect3::getNodeLabelsString(const QStringList &labels) {
    QStringList labels2;
...
QColor NodeRect3::getHighlightBoxColor(const QColor &color) {
    int h, s, v;
    color.getHsv(&h, &s, &v);
...
QMap<NodeRect3::InputVar, QString> NodeRect3::inputVariableNames() {
...
QMap<NodeRect3::DependentVar, QString> NodeRect3::dependentVariableNames() {
    QMap<NodeRect3::DependentVar, QString> result;
