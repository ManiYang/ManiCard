...
NodeRect2::NodeRect2(const int cardId, QGraphicsItem *parent)
    : BoardBoxItem(parent)
    , cardId(cardId)
...
void NodeRect2::setNodeLabels(const QStringList &labels) {
    nodeLabels = labels;
...
void NodeRect2::setTitle(const QString &title) {
...
void NodeRect2::setEditable(const bool editable) {
    titleItem->setEditable(editable);
    textEdit->setReadOnly(!editable);
}

void NodeRect2::setTextEditorIgnoreWheelEvent(const bool b) {
...
QString NodeRect2::getTitle() const {
    return titleItem->toPlainText();
}

QString NodeRect2::getText() const {
...
QMenu *NodeRect2::createCaptionBarContextMenu() {
...
void NodeRect2::setUpContents(QGraphicsItem *contentsContainer) {
